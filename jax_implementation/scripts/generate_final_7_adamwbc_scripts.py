...
epsilon_configs = {
    7.0: {
        "eps_vals": [5e-8, 1e-8],
        "learning_rates": [0.0006, 0.0008, 0.001, 0.0015, 0.002],
...
#SBATCH -o results/output_adamwbc_{epsilon}_wd{weight_decay}_%j.out        # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e results/output_adamwbc_{epsilon}_wd{weight_decay}_%j.err        # File to which STDERR will be written, %j inserts jobid
...
for epsilon, config in epsilon_configs.items():
    ...
    for wd in weight_decays:
        filename = f"run_sweep_eps{epsilon}_wd{wd}_adamwbc.sh"
