...
epsilon_configs = {
    1.0: {
        ...
        "learning_rates": [0.0025, 0.003, 0.0035, 0.004, 0.0045, 0.005],
...
#SBATCH -o results/output_adamw_larger_{epsilon}_wd{weight_decay}_%j.out        # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e results/output_adamw_larger_{epsilon}_wd{weight_decay}_%j.err        # File to which STDERR will be written, %j inserts jobid
...
for epsilon, config in epsilon_configs.items():
    ...
    for wd in weight_decays:
        filename = f"run_sweep_eps{epsilon}_wd{wd}_adamw_larger.sh"
