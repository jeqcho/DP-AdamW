...
epsilon_configs = {
    1.0: {
        ...
        "learning_rates": [0.00007, 0.0001, 0.00013],
    ...
    7.0: {
        "eps_vals": [5e-8, 1e-8],
        "learning_rates": [0.0003, 0.0004, 0.0005],
...
#SBATCH -o results/output_sweep_{epsilon}_wd{weight_decay}_%j.out        # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e results/output_sweep_{epsilon}_wd{weight_decay}_%j.err        # File to which STDERR will be written, %j inserts jobid
...
bias_correction_modes=(true false) # true for --adam_corr, false otherwise
...
do
    ...
    do
        ...
        do
            for bc_mode in "${{bias_correction_modes[@]}}"
            do
                ...
                if [ "$bc_mode" = true ]; then
                    # AdamW-BC (with bias correction)
                    trainer_args="--trainer DPAdamW --adam_corr"
                    exp_suffix="AdamWBC"
                else
...
for epsilon, config in epsilon_configs.items():
    ...
    for wd in weight_decays:
        filename = f"run_sweep_eps{epsilon}_wd{wd}.sh"
