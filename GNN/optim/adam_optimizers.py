...
def scale_by_adam_corr(
    ...
    def init_fn(params):
        ...
        nu_corr = jax.tree_util.tree_map(
    ...
    def update_fn(updates, state, params=None):
        ...
        num_corr = jnp.sum(
            tree_flatten_1dim(
                jax.tree_map(lambda x: jnp.sum((x - noise_err) > (eps_root)), nu_hat)
            )
        )
        ...
        dummy_count = jnp.sum(
            tree_flatten_1dim(jax.tree_map(lambda x: jnp.sum(~jnp.isnan(x)), nu_hat))
        )
