...
def dp_aggregate(
    ...
    def init_fn(params):
        del params
        return optax.DifferentiallyPrivateAggregateState(rng_key=init_rng)
    ...
    def update_fn(summed_updates, state, params):
        ...
        noisy_updates = jax.tree_map(
            lambda g, noise: (g + noise), summed_updates, noise
        )
...
def dpsgd(
    ...
    return optax.chain(
        dp_aggregate(
            batch_size, l2_norm_threshold, base_sensitivity, noise_multiplier, init_rng
        ),
...
def dpadamcorr(
    ...
    return optax.chain(
        ...
        adamcorr(sigma, learning_rate, b1, b2, 0, eps_root),
    )
