...
@jax.jit
def compute_logits(state, graph):
...
def compute_loss(logits, labels):
    ...
    assert labels.shape == logits.shape, (
        f"Got incompatible shapes: logits as {logits.shape}, labels as {labels.shape}"
    )
...
@jax.jit
def compute_loss_multilabel(logits, labels):
    """Computes the mean softmax cross-entropy loss or sigmoid cross-entropy if multilabel."""
    assert labels.shape == logits.shape, (
        f"Got incompatible shapes: logits as {logits.shape}, labels as {labels.shape}"
    )
    loss = jnp.sum(
        optax.sigmoid_binary_cross_entropy(logits=logits, labels=labels), axis=-1
    )
    loss = jnp.mean(loss)
    return loss


def get_subgraphs(graph, pad_to):
    ...
    for node in outgoing_edges:
        ...
        subgraph_indices = subgraph_indices[
            np.sort(np.unique(subgraph_indices, return_index=True)[1])
        ]
        subgraph_indices = subgraph_indices[:pad_to]
        subgraphs[node] = np.pad(
            subgraph_indices,
            (0, pad_to - len(subgraph_indices)),
            "constant",
...
def make_subgraph_from_indices(
    graph, subgraph_indices, add_reverse_edges, adjacency_normalization
):
    ...
    subgraph_nodes = jnp.where(valid_mask[:, jnp.newaxis], subgraph_nodes, 0.0)
    ...
    return normalizations.normalize_edges_with_mask(
        subgraph, valid_mask, adjacency_normalization
    )


@jax.jit
def compute_updates(state, graph, labels, node_indices):
    """Computes gradients for a single batch."""

    def loss_fn(params, graph, labels, node_indices):
...
def reshape_before_pmap(arr):
    ...
    return arr.reshape(
        (
            ...
            arr.shape[0] // jax.local_device_count(),
            *arr.shape[1:],
        )
    )
...
@jax.jit
def get_dp_summed_grads(per_example_gradients, l2_norm_threshold):
    """Requires per example gradients that are normalized by batch size. Standard clipping by L2 norm."""

    grad_norms = jax.tree_map(jax.vmap(jnp.linalg.norm), per_example_gradients)
    divisors = jax.tree_map(
        lambda g_norm: jnp.maximum(g_norm / l2_norm_threshold, 1.0), grad_norms
    )
    clipped_updates = jax.tree_map(
        jax.vmap(lambda g, div: g / div), per_example_gradients, divisors
    )
...
def compute_updates_for_dp(
    ...
    per_example_grads = per_example_gradient_fn(
        state.params, graph, node_labels, subgraphs
    )
    # Batch-normalize
    normed_per_example_grads = jax.tree_map(
        lambda grad: grad / batch_size, per_example_grads
    )
    # Clip and sum
...
def create_model(config, graph, rng):
    """Creates the model and initial parameters."""
    if config.model == "mlp":
        model = models.GraphMultiLayerPerceptron(
            dimensions=(
                [config.latent_size] * config.num_layers + [config.num_classes]
            ),
...
def create_optimizer(
    ...
    if config.differentially_private_training:
        ...
        sigma = optimizers.compute_opt_noise(
            config.l2_norm_threshold, base_sensitivity, config.training_noise_multiplier
        )
        if wandb_logging:
...
def create_train_state(
    ...
    tx = create_optimizer(
        apply_fn, params, config, graph, labels, subgraphs, opt_rng, wandb_logging
    )
    return train_state.TrainState.create(apply_fn=apply_fn, params=params, tx=tx)


def get_max_training_epsilon(config):
...
def compute_metrics(logits, labels, masks, multilabel=False):
    if multilabel:
        train_loss, train_f1_micro = evaluate_predictions_multilabel(
            logits, labels, masks["train"]
        )
        val_loss, val_f1_micro = evaluate_predictions_multilabel(
            logits, labels, masks["validation"]
        )
        test_loss, test_f1_micro = evaluate_predictions_multilabel(
            logits, labels, masks["test"]
        )
        return {
        ...
        train_loss, train_accuracy = evaluate_predictions(
            logits, labels, masks["train"]
        )
        val_loss, val_accuracy = evaluate_predictions(
            logits, labels, masks["validation"]
        )
        test_loss, test_accuracy = evaluate_predictions(logits, labels, masks["test"])
        return {
            "train_loss": train_loss,
...
def log_metrics(
    step, metrics, adam_summary_stats, summary_writer, wandb_logging, postfix=""
):
    """Logs all metrics."""
    ...
    metrics = {metric + postfix: metric_val for metric, metric_val in metrics.items()}
...
def train_and_evaluate(config, workdir):
    ...
    if config.wandb_project is not None:
        ...
        try:
            ...
            if not wandb.run:
              print("not initialized yet. initializing...")
              wandb.init(
            ...
            wandb_logging = False
    print("Logging for wandb is done setup")
    # save config to wandb too
    # TODO
    ...
    state = create_train_state(
        init_rng, config, graph, train_labels, train_subgraphs, wandb_logging
    )
    ...
    for step in range(initial_step, config.num_training_steps):
        ...
        with jax.profiler.StepTraceAnnotation("train", step_num=step):
            ...
            if config.multi_graph:
                graph_indices = jax.random.choice(
                    step_rng, train_graph_index.shape[0], (config.batch_size,)
                )
                indices = train_graph_index[graph_indices, :].astype(int)
                for i in range(indices.shape[0]):
                    if subgraphs is None:
                        subgraphs = jnp.asarray(
                            train_subgraphs[indices[0, 0] : indices[0, 1]]
                        )
                    else:
                        subgraphs = jnp.concatenate(
                            (subgraphs, train_subgraphs[indices[i, 0] : indices[i, 1]])
                        )
                    if batch_labels is None:
                        batch_labels = train_labels[indices[0, 0] : indices[0, 1]]
                    else:
                        batch_labels = jnp.concatenate(
                            (batch_labels, train_labels[indices[i, 0] : indices[i, 1]])
                        )
                del graph_indices, indices
            else:
                indices = jax.random.choice(
                    step_rng, num_training_nodes, (config.batch_size,)
                )
            ...
            for i in range(virtual_batch_num):
                ...
                v_grads = compute_updates_for_dp(
                    state,
                    graph,
                    batch_label_indices,
                    subgraphs_indices,
                    config.adjacency_normalization,
                    config.l2_norm_threshold,
                    batch_size,
                    config.multilabel,
                )
                if grads is None:
        ...
        if step % config.evaluate_every_steps == 0 or is_last_step:
            with report_progress.timed("eval"):
                ...
                if (
                    max_training_epsilon is not None
                    and training_epsilon >= max_training_epsilon
                ):
                    break
                ...
                metrics_during_training = compute_metrics(
                    logits, labels, masks, config.multilabel
                )
        ...
        if (
            ...
            and (step + 1) % config.resample_every_steps == 0
        ):
            ...
            if config.differentially_private_training:
                ...
                del subgraphs
            logging.info(
                "resampling graph: node change %d -> %d, orginal graph has %d nodes",
                old_num_training_nodes,
                num_training_nodes,
                np.shape(base_graph.node_features)[0],
            )
        # garbage collect
        gc.collect()
