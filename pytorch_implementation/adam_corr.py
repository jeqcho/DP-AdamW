...
def adamw_dp(params, grads, exp_avgs, exp_avg_sqs, state_steps, *,
             beta1, beta2, lr, weight_decay, eps,
             dp_batch_size, dp_noise_multiplier, dp_l2_norm_clip):
    """
    Performs one DP-AdamW update without bias correction assuming that the gradients are already clipped and noised
    """
...
class DPAdamW(Optimizer):
    """
    DPAdamW optimizer class without bias correction
    """
    def __init__(
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            adamw_dp(ps, gs, ms, vs, steps, beta1=b1, beta2=b2, lr=group['lr'], weight_decay=group['weight_decay'], eps=group['eps'], dp_batch_size=self.dp_batch_size, dp_noise_multiplier=self.noise_multiplier, dp_l2_norm_clip=self.dp_l2_norm_clip)
        return loss
